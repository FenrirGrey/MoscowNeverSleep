...
def log_load():
    ...
    fail_log = """//srvlis.labquest.local/ExcelImport/Input_Error/""" + name + "-FAIL.log"
    ok_log = """//srvlis.labquest.local/ExcelImport/Input_Error/""" + name + "-OK.log"
    if gl_lang == 'rus':
        dst2 = """//srvlis.labquest.local/ExcelImport/Input/Test/""" + name
    else:
        dst2 = """//srvlis.labquest.local/ExcelImport/Input/Test_ENG//""" + name
    try:
...
def write_file(excel_df, lang, filepath):
    ...
    dst = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    if lang == 'rus':
        dst = """//srvlis.labquest.local/ExcelImport/Input/Test"""
    else:
        dst = """//srvlis.labquest.local/ExcelImport/Input/Test_ENG"""
...
def open_file():
    def check_data(excel_df, lang, filepath):
        ...
        #первая проверка серии, номера паспорта и снилс
        def check_doc(excel_df):
        ...
        #вторая проверка на удаление .0
        def check_doc2(excel_df):
            #Правим СНИЛС
            excel_df['СНИЛС'] = excel_df['СНИЛС'].astype(object)
            val = excel_df['СНИЛС']
            for i in range (0,len(val)):
            ...
            #Правим номер документа
            excel_df['Номер документа'] = excel_df['Номер документа'].astype(object)
            val = excel_df['Номер документа']
            for i in range (0,len(val)):
                a = str(val[i])
                b = a.replace(".0", "")
                val[i] = b           
            excel_df['Номер документа'] = val

        #третья проверка на удаление nan
        def check_doc3(excel_df):
            #Правим СНИЛС
            excel_df['СНИЛС'] = excel_df['СНИЛС'].astype(object)
            val = excel_df['СНИЛС']
            for i in range (0,len(val)):
        ...
        #правим колонку Место работы
        excel_df['Место работы'] = excel_df['Место работы'].astype(object)
        val = excel_df['Место работы']
        for i in range (0,len(val)):
            a = str(val[i])
            b = a.replace("nan", "-")
            val[i] = b
        excel_df['Место работы'] = val 
        log_txt.insert(INSERT, 'Данные исправлены. Копируем файл в необходимую папку\n\n')               
        write_file(excel_df,lang,filepath)

        #правим колонку Пол
        excel_df['Пол'] = excel_df['Пол'].astype(object)
        val = excel_df['Пол']
        for i in range (0,len(val)):
            a = str(val[i])
            b = a.replace("F", "Ж")
        ...
        #правим колонку Дата рождения
        excel_df['Дата рождения'] = excel_df['Дата рождения'].astype(object)
        val = excel_df['Дата рождения']
        for i in range (0,len(val)):
            a = str(val[i])
            b = a.replace("nan", "01.01.1900")
            a = b.replace(" ", ".")
            if a[4] == "-":
                a = a[8] + a[9] + '.' + a[5] + a[6] + '.' + a[0:4]
            val[i] = a
        excel_df['Дата рождения'] = val 
